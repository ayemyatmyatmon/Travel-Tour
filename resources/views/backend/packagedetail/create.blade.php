...
<div class="card">
    <div class="card-body">
        <form action="{{route('packagedetail.store')}}" method="POST" id="create-form" enctype="multipart/form-data">
            @csrf
            <div class="input-group mb-3">
                <select class="custom-select"  name="package_id" id="select-box">
                  <option selected>Choose Town</option>
                    @foreach ($packages as $package )
                        <option value="{{$package->id}}">{{$package->duration}}({{$package->from_to_city}})</option>
                    @endforeach
                </select>
            </div>
